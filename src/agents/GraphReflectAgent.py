...
from src.graph_prompts import GRAPH_DEFINITION, REFLECTION_HEADER, LAST_TRIAL_HEADER, REFLECTION_AFTER_LAST_TRIAL_HEADER
from src.graph_fewshots import EXAMPLES, REFLECT_EXAMPLES
...
class GraphReflectAgent:
    def __init__(self,
        ...
        elif args.llm_version in ["../model/Qwen2.5-7B-Instruct", "/data/hub/models--Qwen--Qwen3-8B/snapshots/9c925d64d72725edaf899c6cb9c377fd0709d9c5"]:
        ...
        elif args.llm_version == "gpt4-mini":
            # GPT-4 mini 설정
            self.llm = OpenAI(
                api_key=os.getenv("OPENAI_API_KEY")
            )
            self.enc = tiktoken.encoding_for_model("gpt-4")
            print("✅ GPT-4 mini 초기화 완료")
    ...
    def run(self, question, answer, reset = True) -> None:
        ...
        while not self.is_correct() and self.round_n <= int(self.max_reflect): #/is_correct_LLM
            ...
            # 첫 번째 답변 저장
            if self.round_n == 1 and self.answer_first == '':
    ...
    def prompt_agent(self) -> str:
        ...
        elif self.llm_version in ['../model/Qwen2.5-7B-Instruct', '/data/hub/models--Qwen--Qwen3-8B/snapshots/9c925d64d72725edaf899c6cb9c377fd0709d9c5']:
            pad_token_id = self.enc.pad_token_id if self.enc.pad_token_id is not None else self.enc.eos_token_id
            eos_token_id = self.enc.eos_token_id
            mid_ans = self.llm(
                    self._build_agent_prompt()[1].content,
        ...
        elif self.llm_version == "gpt4-mini":
            # For GPT-4 mini, use OpenAI API
            try:
                # Convert LangChain messages to OpenAI format
                prompt_messages = self._build_agent_prompt()
                openai_messages = []
                ...
                for msg in prompt_messages:
                    if hasattr(msg, 'type'):
                        ...
                        openai_messages.append({"role": "user", "content": str(msg.content) if hasattr(msg, 'content') else str(msg)})
                ...
                print(f"Error with GPT-4 mini: {e}")
