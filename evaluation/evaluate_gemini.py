...
# Output (JSON format only)"""
    ...
    async def evaluate_single_result(
        ...
        """Evaluate a single result with Gemini"""
    ...
    async def evaluate_file(
        ...
        logger.info(f"Starting Gemini evaluation of {file_path}")
        ...
        logger.info(
            f"Gemini evaluation completed for {file_path}. Results saved to {output_path}"
        )


async def main():
    """Main function to evaluate all result files with Gemini"""
    ...
    os.makedirs("llm_evaluations/gemini", exist_ok=True)
    ...
    for file_path in files_to_evaluate:
        if os.path.exists(file_path):
            ...
            output_path = f"llm_evaluations/gemini/{base_name}_gemini_evaluated.json"
